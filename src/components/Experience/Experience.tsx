...
const Experience = () => {
  ...
  return (
    <section
      id="experience"
      className={`py-20 px-6 ${darkMode ? 'bg-black text-white' : 'bg-white text-gray-900'
        }`}
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className={`absolute left-8 top-0 bottom-0 w-0.5 hidden md:block ${darkMode ? 'bg-slate-700' : 'bg-gray-300'
            }`}></div>
          ...
          <div className="space-y-12">
            {experiences.map((exp) => (
              <div key={exp.id} className="relative flex flex-col md:flex-row gap-8">
                {/* Timeline dot */}
                <div className={`hidden md:flex absolute left-6 w-4 h-4 rounded-full border-4 z-10 ${darkMode
                ...
                <div className="md:ml-20 flex-1">
                  <div className={`p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow ${darkMode ? 'bg-slate-900 border border-slate-800' : 'bg-gray-50 border border-gray-200'
                    }`}>
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                      ...
                      <div className={`px-3 py-1 rounded-full text-sm font-medium mt-2 sm:mt-0 self-start ${darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800'
                        }`}>
                        {exp.period}
                      </div>
                    </div>
                    ...
                    {/* Technologies */}
                    <div className="mb-4">
                      <h5 className={`font-semibold mb-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'
                        }`}>
                        Technologies Used:
                      </h5>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {exp.achievements.map((achievement, achIndex) => (
                          ...
                          >
                            <span className={`mt-1 ${darkMode ? 'text-blue-400' : 'text-blue-600'
                              }`}>•</span>
                            {achievement}
                          </li>
                        ))}
        ...
        <div className="text-center mt-16">
          ...
          <a
            ...
            className={`px-8 py-3 rounded-lg font-semibold transition-colors inline-flex items-center gap-2 ${darkMode
